...
export class StudentModel {
    ...
    static async updateUserStudentData({ studentData }) {

        const { userId, name, lastName, email } = studentData;
        console.log(studentData)

        try {
            await pool.query(
                'UPDATE usuario SET nombre = ?, apellido = ?, correo = ? WHERE usuario_id = ?',
    ...
    // static async updateStudentData({ studentData }) {
    //     const { userId, name, lastName, career, gender, tutorOrParent, birthdate, age, placeOfBirth, religion, activity } = studentData;

    //     try {
    //         await pool.query(
    //             'UPDATE alumno SET carrera = ?, genero = ?, tutor_padre = ?, fechaNac = ?, edad = ?, lugarNac = ?, religion = ?, actividad = ? WHERE usuario_id = ?',
    //             [career, gender, tutorOrParent, birthdate, age, placeOfBirth, religion, activity, userId]
    //         );
    ...
    static async updateContactData({ contactData }) {
        const { userId, currentAddress, homeAddress, cellPhoneNumber, homePhoneNumber, tutorsEmail } = contactData;

        try {
            // Realizar la actualización en la base de datos
            await pool.query(
                'UPDATE contacto SET domicilio_actual = ?, domicilio_familiar = ?, celular = ?, tel_casa = ?, correo_tutor = ? WHERE usuario_id = ?',
                [currentAddress, homeAddress, cellPhoneNumber, homePhoneNumber, tutorsEmail, userId]
            );

            // Consultar los datos actualizados
            const [updatedContactData] = await pool.query(
                'SELECT * FROM contacto WHERE usuario_id = ?',
                [userId]
            );

            // Verificar si se encontraron datos actualizados
            if (updatedContactData.length > 0) {
                return updatedContactData[0]; // Devolver los datos actualizados
            } else {
                throw new Error('No se encontraron datos actualizados');
            }
        } catch (error) {
            console.log(error);
            throw new Error('Error al actualizar los datos de contacto');
        }
    }

    static async updateMedicalData({ medicalData }) {
        const { userId, socialSecurityNumber, bloodType, disease, disability, allergy, sustances } = medicalData;

        try {
            await pool.query(
                'UPDATE datos_medicos SET num_seguro = ?, tipo_sangre = ?, enfermedad = ?, discapacidad = ?, alergia = ?, sustancias_toxicas = ? WHERE usuario_id = ?',
                [socialSecurityNumber, bloodType, disease, disability, allergy, sustances, userId]
            );

            return { message: 'Datos médicos actualizados correctamente' };
        } catch (error) {
            console.log(error);
            throw new Error('Error al actualizar los datos médicos');
        }
    }

    static async updateAcademicData({ academicData }) {
        const { userId, highSchool, average, scoreCeneval } = academicData;

        try {
            await pool.query(
                'UPDATE datos_academicos SET preparatoria = ?, promedio = ?, puntuacion_ceneval = ? WHERE usuario_id = ?',
                [highSchool, average, scoreCeneval, userId]
            );

            return { message: 'Datos académicos actualizados correctamente' };
        } catch (error) {
            console.log(error);
            throw new Error('Error al actualizar los datos académicos');
        }
    }

    static async updateSocioeconomicData({ socioeconomicData }) {
        const { userId, workplace, economicalSupport, livesWith } = socioeconomicData;

        try {
            await pool.query(
                'UPDATE datos_socioeconomicos SET trabajo = ?, apoyo_economico = ?, convivencia = ? WHERE usuario_id = ?',
                [workplace, economicalSupport, livesWith, userId]
            );

            return { message: 'Datos socioeconómicos actualizados correctamente' };
        } catch (error) {
            console.log(error);
            throw new Error('Error al actualizar los datos socioeconómicos');
        }
    }
    ...
    static async deleteStudent(id) {
        ...
        try {
            ...
            await connection.query(`DELETE FROM tutor_alumno WHERE alumno_id = ?`, [id]);
            await connection.query(`DELETE FROM contacto WHERE usuario_id = ?`, [id]);
            await connection.query(`DELETE FROM datos_academicos WHERE usuario_id = ?`, [id]);
            await connection.query(`DELETE FROM datos_medicos WHERE usuario_id = ?`, [id]);
            await connection.query(`DELETE FROM datos_socioeconomicos WHERE usuario_id = ?`, [id]);
            await connection.query(`DELETE FROM imagen WHERE usuario_id = ?`, [id]);
            await connection.query(`DELETE FROM alumno WHERE usuario_id = ?`, [id]);
            const [result] = await connection.query(`DELETE FROM usuario WHERE usuario_id = ?`, [id]);
